...
class Orders extends CI_Controller {
    ...
    public function view($id = 0)
    {
        if ( $this->order->is_order_exist($id))
        {
            ...
            $params['title'] = 'Pesanan '. $data->order_number;
        ...
        switch ($action)
        {
            case 'cancel_order' :
                ...
                if ( ($data->payment_method == 1 && $data->order_status == 1) || ($data->payment_method == 2 && $data->order_status == 1))
                {
                    ...
                    $response = array('code' => 200, 'success' => TRUE, 'message' => 'Pesanan dibatalkan');
                }
                else
                {
                    $response = array('code' => 200, 'error' => TRUE, 'message' => 'Pesanan tidak dapat dibatalkan');
                }
            break;
            case 'delete_order' :
                $id = $this->input->post('id');
                ...
                if ( ($data->payment_method == 1 && ($data->order_status == 5 || $data->order_status == 4)) || ($data->payment_method == 2 && ($data->order_status == 4 || $sata->order_status == 3)))
                {
                    ...
                    $response = array('code' => 200, 'success' => TRUE, 'message' => 'Pesanan dihapus');
                }
                else
                {
                    $response = array('code' => 200, 'error' => TRUE, 'message' => 'Pesanan tidak dapat dihapus');
                }
            break;
        }
